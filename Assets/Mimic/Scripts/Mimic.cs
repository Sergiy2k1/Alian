...
namespace MimicSpace
{
    public class Mimic : MonoBehaviour
    {
        ...
        void Start()
        {
            // Встановіть маску шару, яка ігнорує шар Player
        ...
        void Update()
        {
            ...
            if (legCount <= maxLegs - partsPerLeg)
            {
                ...
                RaycastHit hit;
                if (Physics.Raycast(newLegPosition + Vector3.up * 10f, -Vector3.up, out hit, Mathf.Infinity, ignoreLayerMask))
                {
                    Vector3 myHit = hit.point;
                    if (Physics.Linecast(transform.position, hit.point, out hit, ignoreLayerMask))
                        myHit = hit.point;
